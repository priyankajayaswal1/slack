...
module.exports = {
  async login(req, res) {
    // FIXME: make dynamic
    const scope = 'links:read,links:write,commands,bot,chat:write:bot,team:read';
  ...
  async callback(req, res) {
    ...
    const [workspace, created] = await SlackWorkspace.findOrCreate({
      ...
      defaults: { accessToken: access.access_token, botAccessToken: access.bot.bot_access_token },
    });

    if (!created) {
      await workspace.update({
        accessToken: access.access_token,
        botAccessToken: access.bot.bot_access_token,
      });
    }
    ...
    await workspace.botClient.chat.postMessage({
      channel: access.user_id,
      as_user: true,
      ...new SlackInstalled().toJSON(),
    });

    return res.redirect(`${slackRootUrl}/app_redirect?app=${process.env.SLACK_APP_ID}&team=${access.team_id}`);
  },
};
